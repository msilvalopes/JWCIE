...
public class remover extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if ((String) request.getParameter("senha") != null) {

            Usuarios user = sessao.getUsuarioLogado();
            UsuariosDAO dao = new UsuariosDAO();
            DiretorioManager dir = new DiretorioManager(user.getLogin());
            
            if(user.senhaIgual(request.getParameter("senha"))){
                dao.delete(user);
                dir.eliminar_diretorio();
                sessao.encerrar(request, response);
            }
        }else{
            request.setAttribute("usuario", sessao.getUsuarioLogado());
            request.getRequestDispatcher("remover_conta.jsp").forward(request, response);
        }
    }
